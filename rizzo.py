    ...
    def match(self, extsigs):
        ...
        for (extsig, ext_func_ea) in extsigs.formal.iteritems():
            if self.signatures.formal.has_key(extsig):
                ...
                print "Found formal matches %s at %x." % (curfunc.name, newfunc.ea)
        end = time.time()
        ...
        for (extsig, ext_func_ea) in extsigs.fuzzy.iteritems():
            if self.signatures.fuzzy.has_key(extsig):
                ...
                if len(curfunc.blocks) == len(newfunc.blocks):
                    ...
                    print "Found fuzzy matches %s at %x." % (newfunc.name, curfunc.ea)
        ...
        for (extsig, ext_func_ea) in extsigs.strings.iteritems():
            if self.signatures.strings.has_key(extsig):
                ...
                print "Found strings matches %s at %x." % (newfunc.name, curfunc.ea)
        end = time.time()
        ...
        for (extsig, ext_func_ea) in extsigs.immediates.iteritems():
            if self.signatures.immediates.has_key(extsig):
                ...
                print "Found immediates matches %s at %x." % (newfunc.name, curfunc.ea)
        end = time.time()
...
try:
    ...
    class Rizzo_Menu_Context(idaapi.action_handler_t):
    ...
    class Rizzo_MC_Load(Rizzo_Menu_Context):
        def activate(self, ctx):
            self.plugin.rizzo_load(True)
            return 1

    # context menu for produce
    class Rizzo_MC_Produce(Rizzo_Menu_Context):
        def activate(self, ctx):
            self.plugin.rizzo_produce(True)
            return 1
